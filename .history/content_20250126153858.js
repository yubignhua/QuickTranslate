...
speechSynthesis.addEventListener('voiceschanged', () => {
  ...
  for (const voice of voices) {
    // 优先使用 Google 的声音
    if (voice.name.includes('Google') && voice.lang.includes('zh-CN')) {
      chineseVoice = voice;
    }
    if (voice.name.includes('Google') && voice.lang.includes('en-US')) {
      englishVoice = voice;
    }
  }
  
  // 如果没有找到 Google 的声音，使用系统默认的
  if (!chineseVoice) {
    chineseVoice = voices.find(v => v.lang.includes('zh-CN'));
  }
  if (!englishVoice) {
    englishVoice = voices.find(v => v.lang.includes('en-US'));
  }
  
  console.log('已加载语音:', { 
    chinese: chineseVoice?.name, 
    english: englishVoice?.name 
  });
});
