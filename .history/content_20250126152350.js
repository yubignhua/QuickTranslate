...
function adjustPosition(x, y, width, height) {
  ...
  // 确保不超出右边界
  if (x + width > scrollX + viewportWidth) {
    x = scrollX + viewportWidth - width - 10;
  }
  ...
  // 确保不超出底部边界
  if (y + height > scrollY + viewportHeight) {
    y = scrollY + viewportHeight - height - 10;
  }
...
document.addEventListener('mouseup', (e) => {
  ...
  requestAnimationFrame(() => {
    ...
    try {
      ...
      // 计算图标位置
      let iconX = selectionRect.right + window.scrollX + 5;
      let iconY = selectionRect.top + window.scrollY - 20;
      
      // 如果选中区域在屏幕右侧，将图标显示在左侧
      if (iconX > window.innerWidth - 50) {
        iconX = selectionRect.left + window.scrollX - 29;
      }
      
      // 如果选中区域在屏幕顶部，将图标显示在下方
      if (iconY < window.scrollY) {
        iconY = selectionRect.bottom + window.scrollY + 5;
      }
